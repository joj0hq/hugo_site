...
株式会社 10X にソフトウェアエンジニアとして入社。小規模な分散チームで、日本をリードするオンラインスーパープラットフォームを構築しています。ストア開発、ML、プラットフォームグループで働き、GCP上でバックエンドとFlutter開発をリードしています。

ML駆動のランキングバックエンドを設計・実装し、製品発見をエンドツーエンドで支えています。API設計、データモデリング、クライアント統合まで担当し、厳しいレイテンシと信頼性要件を満たしています。

製品取得アーキテクチャとFirestoreスキーマを再設計し、テールレイテンシを大幅に削減し、幅広い製品カテゴリにスケールできるようにしました。フィーチャーフラグを使用して安全に変更を展開しています。機能提供を超えて、要件からリリースまでプロジェクトを所有し、コードレビューやテストの実践を定義し、多くのPRをレビューしながら、技術的制約とビジネス・UXへの影響のバランスを取っています。
